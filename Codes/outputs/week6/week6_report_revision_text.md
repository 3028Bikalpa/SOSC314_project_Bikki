...
The final selected model is Logistic Regression with TF-IDF (10,000 features, unigrams+bigrams, class_weight='balanced', C=1.0, solver='liblinear'). Stopping criterion: stop iterating when validation improvement <=0.005 across two consecutive updates or when additional complexity reduces generalization.
...
3. External validity limitation: data come from a single platform (RateMyProfessors), so transferability to other educational contexts is uncertain without external validation.
